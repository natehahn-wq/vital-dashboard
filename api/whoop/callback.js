...
// == Helper: fetch zone_durations from individual workout detail endpoint ==
// Note: WHOOP v2 API uses "zone_durations" (plural) not "zone_duration"
async function fetchWorkoutZones(workoutId, headers) {
      try {
              ...
              const zd = detail.score?.zone_durations || detail.score?.zone_duration || null;
              console.log(`[VITAL] Workout ${workoutId} zone_durations:`, JSON.stringify(zd));
              return zd;
      } catch (e) {
...
// == Helper: compute 6-zone percentages + minutes from zone_durations ==
// Handles both possible field naming conventions from WHOOP API
function computeZones(zd, totalDurMs) {
      if (!zd || !totalDurMs || totalDurMs <= 0) {
              return { z0p:0, z1p:0, z2p:0, z3p:0, z4p:0, z5p:0, z0m:0, z1m:0, z2m:0, z3m:0, z4m:0, z5m:0 };
      }
      // Try both naming conventions: zone_zero_milli OR zone_one_milli style
  // Also handle possible array format
  let z0ms, z1ms, z2ms, z3ms, z4ms, z5ms;
      if (Array.isArray(zd)) {
              z0ms = zd[0] || 0;
              z1ms = zd[1] || 0;
              z2ms = zd[2] || 0;
              ...
              z0ms = zd.zone_zero_milli  || zd.zone_0_milli || 0;
              z1ms = zd.zone_one_milli   || zd.zone_1_milli || 0;
              z2ms = zd.zone_two_milli   || zd.zone_2_milli || 0;
              z3ms = zd.zone_three_milli || zd.zone_3_milli || 0;
              z4ms = zd.zone_four_milli  || zd.zone_4_milli || 0;
              z5ms = zd.zone_five_milli  || zd.zone_5_milli || 0;
      }
      return {
...
export async function fetchAndStoreWhoopData(accessToken) {
  ...
  // Fetch individual workout details in parallel to get zone_durations
  ...
  const normalized = {
          ...
          workouts: workouts.slice(0, 5).map((w, i) => {
                    ...
                    const zd = workoutDetails[i] || w.score?.zone_durations || w.score?.zone_duration || null;
                    const zones = computeZones(zd, totalDurMs);
                    return {
                                id:     w.id,
                                sport:  w.sport_id,
  ...
  console.log('[VITAL] WHOOP data stored:', normalized.date,
                  ...
                  'workout zones:', normalized.workouts.map(w => `${w.id.slice(0,8)}:z0=${w.zones.z0p}%,z1=${w.zones.z1p}%`).join(' | '));
      return normalized;
}
